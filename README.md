...
2. **`customer_churn_db` (Almacenamiento):** Motor relacional `postgres:16-alpine` mapeado externamente al puerto **`5501`** (interno `5432`). Mantiene persistencia de datos mediante volúmenes de Docker (`pgdata_churn`) e implementa restricciones de integridad y vistas analíticas de negocio en caliente. Credenciales externalizadas mediante `.env`.
3. **`customer_churn_streamlit` (Dashboard Ejecutivo):** Panel corporativo interactivo en gama de azules desarrollado con Streamlit mapeado externamente al puerto **`8601`** (interno `8501`) y Plotly. Permite la exploración visual de métricas y simulaciones de inferencia unitaria en tiempo real mediante una lógica híbrida (GPU local / CPU en la nube con Neon Postgres).

---

## 🗺️ Plan Maestro de Direccionamiento de Red (Estandarizado)

Para garantizar la coexistencia simultánea de múltiples entornos sin colisiones en el Host local, se implementa un esquema de aislamiento perimetral basado en los rangos corporativos **`5500+` (SQL)** y **`8600+` (Web UI)**:

| Servicio Docker   | Puerto Externo (Host Ubuntu) | Puerto Interno (Contenedor) | Acceso de Desarrollo (Dev Container) |
| :---------------- | :--------------------------: | :-------------------------: | :----------------------------------- |
| `db_churn`        |          **`5501`**          |           `5432`            | Host: `db_churn` \| Puerto: `5432`   |
| `streamlit_churn` |          **`8601`**          |           `8501`            | URL Host: `http://localhost:8601`    |
